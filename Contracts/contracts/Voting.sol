...
contract Voting {
    ...
    function removeAllCandidates() public {
        ...
        require(!votingStarted, "Cannot remove candidates while voting is active");

        for (uint i = 0; i < candidateList.length; i++) {
            delete candidates[candidateList[i].candidateAddress];
        }
        candidateList.pop();
        emit success("All candidates removed!!");
    }
